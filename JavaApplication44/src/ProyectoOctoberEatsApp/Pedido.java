...
public class Pedido {
    ...
    private int puerto = 1234; // El puerto donde el servidor está escuchando
    ...
    private int pedidosContador = 0; // Asegúrate de que esta variable sea un miembro de la clase

    private void actualizarPedidosTable(String numeroCompra) {
        SwingUtilities.invokeLater(() -> {
            DefaultTableModel model = (DefaultTableModel) pedidosTable.getModel();
            model.addRow(new Object[]{numeroCompra}); // Añade el nuevo pedido a la tabla
            pedidosContador++; // Incrementa el contador de pedidos
            pedidosTotal.setText(Integer.toString(pedidosContador)); // Actualiza el JLabel con el nuevo valor
        });
    }
...
// Método para manejar la acción de mover un pedido a "En proceso"
    private void moverAEnProceso() {
        ...
        if (filaSeleccionada >= 0) {
            ...
            // Obtiene el valor de la fila seleccionada de pedidosTable
            Object pedido = modelPedidos.getValueAt(filaSeleccionada, 0);

            // Añade el pedido a pedidosP
            modelEnProceso.addRow(new Object[]{pedido});

            // Remueve el pedido de pedidosTable
